...
import com.wistron.pojo.vo.SoftpaqVo;
import com.wistron.pojo.vo.SoftpaqVos;

@Controller
public class SoftpaqController {

	private SoftpaqDaoImpl  daoImple = new SoftpaqDaoImpl();

	/* Partition of Commodity-------------------->Start */
	
	@RequestMapping("/record/softpaq")
	public String softpaq(HttpSession session, Model model) throws ParseException {
		
		 List<Softpaq> listpojo = daoImple.findAll();
		 List<SoftpaqVo> listvos = new ArrayList<SoftpaqVo>();		
	  	  int size = listpojo.size(); 
	  	  for(int i=0;i<size;i++) { 
		  	  Softpaq pojo = listpojo.get(i); 
			  ...
			  SoftpaqVo vo = new SoftpaqVo(pojo.getSoftpaq_id(),pojo.getOwner(),pojo.getChassis(),pojo.getPlatform(),pojo.getSp_number(),pojo.getSoftpaq_title(),pojo.getVersion(),schedule,pojo.getBios_version(),pojo.getImage_build_id(),pojo.getTest_plan(),pojo.getTester());
			  listvos.add(vo);
		...
		return "/WEB-INF/views/softpaq_record.jsp";
	}
...
//		System.out.println(vos);
	...
	@RequestMapping("/record/editsoftpaq")
	public void update(@RequestBody SoftpaqVo vo, HttpSession session, HttpServletResponse response)throws ParseException, IOException {
		User user = (User) session.getAttribute("session_user");
		Date[] startAndEnd = splitSchedule(vo.getSchedule());	
		Softpaq pojo = new Softpaq(vo.getSoftpaq_id(),user.getEnname(),vo.getChassis(),vo.getPlatform(),vo.getSp_number(),vo.getSoftpaq_title(),vo.getVersion(),startAndEnd[0],startAndEnd[1],vo.getBios_version(),vo.getImage_build_id(),vo.getTest_plan(),vo.getTester());
