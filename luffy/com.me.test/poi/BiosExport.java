...
public class BiosExport {
	...
	public void biosExport() throws Exception{
        ...
        int rownum = 1;
        int bisodatasize = data.size();
        for (int i = 0; i < bisodatasize; i++, rownum++) {
...
/*================== 2.commodity ===================  */
		...
		List<Commodity> commoditydata = commodityDao.findAllCommodity();
...
/*================== 3.Softpaq ===================  */
		...
		String[] softpaqHeader = {"Softpaq ID","Owner","Chassis","Platform","SP Number","Softpaq Title","Version","Start","End","BIOS Version","Image Build Id","Test Plan","Tester"};
		/**
      ...
      //get data from database
        SoftpaqDaoImpl softpaqDao = new SoftpaqDaoImpl();
		List<Softpaq> softpaqdata = softpaqDao.findAll();
        ...
        for (int i = 0; i < softpaqdatasize; i++, softpaqrownum++) {
        	...
        	Softpaq softpaq = softpaqdata.get(i);
        	Cell cellId = row.createCell(0);
        	...
        	Cell cellChassis = row.createCell(2);
        	cellChassis.setCellValue(softpaq.getChassis());
        	...
        	Cell cellBIOSVersion = row.createCell(9);
        	cellBIOSVersion.setCellValue(softpaq.getBios_version());
        	
        	Cell cellImageBuildId = row.createCell(10);
        	cellImageBuildId.setCellValue(softpaq.getImage_build_id());
        	
        	Cell cellTestPlan = row.createCell(11);
        	cellTestPlan.setCellValue(softpaq.getTest_plan());
        	
        	Cell cellTester = row.createCell(12);
        	cellTester.setCellValue(softpaq.getTester());
...
/*================== 4.Wat ===================  */
      ...
      //get data from database
        WatDaoImpl watDao = new WatDaoImpl();
		List<Wat> watdata = watDao.findAll();
...
/*================== 5.image softroll&respin ===================  */
        ...
        //get data from database
        SoftrollRespinDaoImpl softrollrespinDao = new SoftrollRespinDaoImpl();
		List<SoftrollRespin> softrollrespindata = softrollrespinDao.findAll();
